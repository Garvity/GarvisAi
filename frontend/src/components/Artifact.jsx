...
function Artifact() {
  ...
  const activeFileObj = artifacts[0]?.files?.[activeFile];
  const html = artifacts[0]?.files?.find(
    (f) => f.name === "index.html",
  )?.content;
  const css = artifacts[0]?.files?.find((f) => f.name === "style.css")?.content;
  const js = artifacts[0]?.files?.find((f) => f.name === "script.js")?.content;
            ...
            <div className="flex-1 overflow-hidden">
              {tab === "preview" && canPreview ? (
                ...
                >
                  <Editor
                    theme="vs-dark"
                    language={detectLanguage(activeFileObj?.name)}
                    value={file?.content}
                    options={{
                      readOnly: true,
